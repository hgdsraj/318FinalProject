...
#Need date column from images, parsed into date/time using prior functions (write_katkam_json i think)
#Also need to join weather column on date/time, and use StringIndexer to label numerically
jobs_df = readImages(img_dir)
jobs_df.show()
df = DeepImageFeaturizer(inputCol="image", outputCol="features", modelName="InceptionV3").fit(jobs_df).transform(jobs_df)
jobs_df.show()
i = 2
...
from pyspark.ml.evaluation import MulticlassClassificationEvaluator
df = lr_model.transform(jobs_test)
df.show()
